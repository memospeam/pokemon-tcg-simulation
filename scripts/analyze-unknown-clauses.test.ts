...
describe("analyze unknown clauses", () => {
  it("prints top unknown clause texts", () => {
    ...
    for (const record of records) {
      ...
      for (const effect of effects) {
        if (effect.kind !== "unknown") continue;
        clauseCounts.set(effect.text, (clauseCounts.get(effect.text) ?? 0) + record.cardCount);
      }
    }
    const top = [...clauseCounts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 60);
    console.log("\nTop unknown clauses:");

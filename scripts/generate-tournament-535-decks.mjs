/**
 * Fetches Regional Utrecht (tournament 535) Masters decklists and writes Top 16 JSON.
 * Usage: node scripts/generate-tournament-535-decks.mjs
 */
...
const TOURNAMENT_ID = 535;
const TOP_N = 16;
const OUT_PATH = join(
  dirname(fileURLToPath(import.meta.url)),
  "../data/tournaments/535-top16.json",
);
...
async function main() {
  ...
  const output = {
    ...
    decks: decks.map((deck) => ({
      ...deck,
      id: `utrecht-${deck.placement}-${deck.player.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`,
